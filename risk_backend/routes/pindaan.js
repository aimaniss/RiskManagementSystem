...
const riskMatrixDetails = {
  1: {
    1: { label: "Rendah", shortLabel: "R", color: "#22c55e", textColor: "#ffffff" },
    2: { label: "Rendah", shortLabel: "R", color: "#22c55e", textColor: "#ffffff" },
    3: { label: "Sederhana", shortLabel: "S", color: "#eab308", textColor: "#854d0e" },
    4: { label: "Sederhana", shortLabel: "S", color: "#eab308", textColor: "#854d0e" },
    5: { label: "Tinggi", shortLabel: "T", color: "#f97316", textColor: "#ffffff" },
  },
  2: {
    1: { label: "Rendah", shortLabel: "R", color: "#22c55e", textColor: "#ffffff" },
    2: { label: "Rendah", shortLabel: "R", color: "#22c55e", textColor: "#ffffff" },
  ...
  3: {
    1: { label: "Rendah", shortLabel: "R", color: "#22c55e", textColor: "#ffffff" },
    2: { label: "Sederhana", shortLabel: "S", color: "#eab308", textColor: "#854d0e" },
    3: { label: "Sederhana", shortLabel: "S", color: "#eab308", textColor: "#854d0e" },
    4: { label: "Tinggi", shortLabel: "T", color: "#f97316", textColor: "#ffffff" },
    5: { label: "Tinggi", shortLabel: "T", color: "#f97316", textColor: "#ffffff" },
  },
  4: {
    1: { label: "Sederhana", shortLabel: "S", color: "#eab308", textColor: "#854d0e" },
    2: { label: "Sederhana", shortLabel: "S", color: "#eab308", textColor: "#854d0e" },
    3: { label: "Tinggi", shortLabel: "T", color: "#f97316", textColor: "#ffffff" },
    4: { label: "Tinggi", shortLabel: "T", color: "#f97316", textColor: "#ffffff" },
    5: { label: "Sangat Tinggi", shortLabel: "ST", color: "#ef4444", textColor: "#ffffff" },
  },
  5: {
    1: { label: "Sederhana", shortLabel: "S", color: "#eab308", textColor: "#854d0e" },
    2: { label: "Tinggi", shortLabel: "T", color: "#f97316", textColor: "#ffffff" },
    3: { label: "Tinggi", shortLabel: "T", color: "#f97316", textColor: "#ffffff" },
    4: { label: "Sangat Tinggi", shortLabel: "ST", color: "#ef4444", textColor: "#ffffff" },
    5: { label: "Sangat Tinggi", shortLabel: "ST", color: "#ef4444", textColor: "#ffffff" },
  },
};
...
router.post("/:risk_id", verifyToken, authorizeKebenaran("pindaan:urus"), async (req, res) => {
  ...
  try {
    ...
    const completeFinalData = {
      skor_kebarangkalian:
        data_selepas.skor_kebarangkalian ?? completeOriginalData.skor_kebarangkalian,
      skor_impak: data_selepas.skor_impak ?? completeOriginalData.skor_impak,
      skor_kebarangkalian_selepas:
        data_selepas.skor_kebarangkalian_selepas ??
        completeOriginalData.skor_kebarangkalian_selepas,
      skor_impak_selepas:
        data_selepas.skor_impak_selepas ?? completeOriginalData.skor_impak_selepas,
    };
    ...
    if (hasPenilaianInputChange) {
      const beforeScore = getRiskShortLabelFromMatrix(
        completeOriginalData.skor_kebarangkalian,
        completeOriginalData.skor_impak,
        riskMatrixDetails
      );
      const afterScore = getRiskShortLabelFromMatrix(
        completeFinalData.skor_kebarangkalian,
        completeFinalData.skor_impak,
        riskMatrixDetails
      );
      if (
        beforeScore !== afterScore ||
        "skor_kebarangkalian" in data_selepas ||
        "skor_impak" in data_selepas
      ) {
    ...
    if (hasKeberkesananInputChange) {
      const beforeScore = getRiskShortLabelFromMatrix(
        completeOriginalData.skor_kebarangkalian_selepas,
        completeOriginalData.skor_impak_selepas,
        riskMatrixDetails
      );
      const afterScore = getRiskShortLabelFromMatrix(
        completeFinalData.skor_kebarangkalian_selepas,
        completeFinalData.skor_impak_selepas,
        riskMatrixDetails
      );
      if (
        beforeScore !== afterScore ||
        "skor_kebarangkalian_selepas" in data_selepas ||
        "skor_impak_selepas" in data_selepas
      ) {
    ...
    if (nama_peranan === "Admin") {
      ...
      if (hasRisikoChanges) {
        if ("skor_kebarangkalian" in data_selepas)
          risikoUpdates.skor_kebarangkalian = data_selepas.skor_kebarangkalian;
      ...
      if (hasLogChanges) {
        if ("skor_kebarangkalian_selepas" in data_selepas)
          logUpdates.skor_kebarangkalian_selepas = data_selepas.skor_kebarangkalian_selepas;
        if ("skor_impak_selepas" in data_selepas)
          logUpdates.skor_impak_selepas = data_selepas.skor_impak_selepas;
        if ("skor_risiko_keberkesanan" in data_selepas) {
          logUpdates.skor_risiko_pemantauan =
            data_selepas.skor_risiko_keberkesanan !== "-"
              ? data_selepas.skor_risiko_keberkesanan
              : null;
        }
      }
      ...
      if (hasLogChanges && Object.keys(logUpdates).length > 0) {
        ...
        if (latestLogRes.rowCount > 0) {
          ...
          const setClause = Object.keys(logUpdates)
            .map(
              (key, index) =>
                `"${key}" = ${key === "update_at_pemantauan" ? logUpdates[key] : `$${index + 1}`}`
            )
            .join(", ");
          ...
          console.warn(
            `Tiada log pemantauan sedia ada untuk risiko ID ${risk_id}. Pindaan keberkesanan tidak dikemaskini.`
          );
        }
      }
    ...
    const newPermohonan = await client.query(insertQuery, [
      risk_id,
      userIntegerId,
      status_permohonan, // Guna ID INTEGER pemohon
      data_sebelum,
      data_selepas, // Hantar objek JSON yang telah dikira skor
      penilaian || null,
      keberkesanan || null,
      pengguna_id_pelulus,
      ...
      if (nama_peranan !== "Admin") {
        ...
        if (adminIds.length > 0) {
          ...
          await hantarNotifikasiBulk(
            ...
            mesej,
            "pindaan_baru",
            newPermohonan.rows[0].pindaan_id
          );
        }
      }
    } catch (notifErr) {
    ...
    try {
      const { rows: risikoRow2 } = await pool.query(
        `SELECT no_rujukan FROM risiko WHERE risiko_id = $1`,
        [risk_id]
      );
  ...
  async (req, res) => {
    ...
    try {
      ...
      const permohonanRes = await client.query(
        "SELECT * FROM permohonan_pindaan WHERE pindaan_id = $1 AND status_permohonan = 'Menunggu Kelulusan'",
        [pindaan_id]
      );
      ...
      const {
        risiko_id,
        data_selepas,
        justifikasi_penilaian,
        justifikasi_keberkesanan,
        pengguna_id_pemohon,
      } = permohonan;

      let hasRisikoChanges =
        ["skor_kebarangkalian", "skor_impak"].some((key) => key in data_selepas) ||
        justifikasi_penilaian;
      let hasLogChanges =
        ["skor_kebarangkalian_selepas", "skor_impak_selepas"].some((key) => key in data_selepas) ||
        justifikasi_keberkesanan;

      // 1. Kemas kini Jadual RISIKO
      if (hasRisikoChanges) {
        /* ... Kod UPDATE Risiko kekal sama ... */
        const risikoUpdates = {};
        if ("skor_kebarangkalian" in data_selepas)
          risikoUpdates.skor_kebarangkalian = data_selepas.skor_kebarangkalian;
        if ("skor_impak" in data_selepas) risikoUpdates.skor_impak = data_selepas.skor_impak;
      ...
      if (hasLogChanges) {
        ...
        if (latestLogRes.rowCount > 0) {
          ...
          if ("skor_risiko_keberkesanan" in data_selepas) {
            ...
            logUpdates.skor_risiko_pemantauan =
              calculatedRiskScoreLog !== "-" ? calculatedRiskScoreLog : null;
          }
          if (justifikasi_keberkesanan)
            logUpdates.justifikasi_pindaan_pemantauan = justifikasi_keberkesanan;

          // Guna ID INTEGER Admin untuk updated_by_pemantauan
          logUpdates.updated_by_pemantauan = adminIntegerId;
          logUpdates.update_at_pemantauan = "NOW()";

          const fieldsToUpdate = Object.keys(logUpdates).filter(
            (k) => k !== "update_at_pemantauan" && k !== "updated_by_pemantauan"
          );
          ...
          console.warn(
            `Tiada log pemantauan sedia ada untuk risiko ID ${risiko_id}. Pindaan keberkesanan tidak dikemaskini semasa kelulusan.`
          );
        }
      }
    ...
    `;
      const updatedPermohonan = await client.query(updatePermohonanQuery, [
        adminIntegerId,
        pindaan_id,
      ...
      try {
        const { rows: pindaanInfo } = await pool.query(
          `SELECT no_rujukan_pindaan FROM permohonan_pindaan WHERE pindaan_id = $1`,
          [pindaan_id]
        );
...
router.put(
  ...
  async (req, res) => {
    ...
    `;
      ...
      try {
        const { rows: pindaanInfo } = await pool.query(
          `SELECT no_rujukan_pindaan, risiko_id FROM permohonan_pindaan WHERE pindaan_id = $1`,
          [pindaan_id]
        );
        const { rows: risikoInfo } = await pool.query(
          `SELECT no_rujukan FROM risiko WHERE risiko_id = $1`,
          [pindaanInfo[0].risiko_id]
        );
        const noRujukanPindaan = pindaanInfo[0]?.no_rujukan_pindaan || `ID ${pindaan_id}`;
