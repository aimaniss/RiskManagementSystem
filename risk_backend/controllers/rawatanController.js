...
/* =======================================================
   ...
   ======================================================= */
export const senaraiRawatan = async (req, res) => {
  try {
    const user = req.user;
    let query = `
SELECT
...
/* =======================================================
   GET: Rawatan WITH Status Pemantauan
   Khusus untuk PenilaianDanRawatan.jsx sahaja
   ======================================================= */
export const senaraiRawatanDenganStatus = async (req, res) => {
  try {
    const user = req.user;
    let query = `
SELECT
...
/* =======================================================
   ...
   ======================================================= */
export const simpanPenilaian = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      console.warn("Gagal update LogPemantauan:", logErr.message);
    }
...
/* =======================================================
   POST: Tambah Rawatan + Update Status
   ======================================================= */
export const tambahRawatan = async (req, res) => {
  const client = await pool.connect();
  const user = req.user;
  ...
  try {
    ...
    // 4. UPDATE STATUS PEMANTAUAN (macam DaftarRisiko.jsx pattern)
    try {
      await client.query(
        `UPDATE LogPemantauan
                 SET status_pemantauan = $1,
...
/* =======================================================
   PUT, DELETE, GET/:risiko_id
   ======================================================= */
export const kemaskiniRawatan = async (req, res) => {
  const client = await pool.connect();
  const user = req.user;
...
export const padamRawatan = async (req, res) => {
  const user = req.user;
  ...
  try {
    ...
    const { noRujukanUntukLog } = await dalamTransaksi(async (client) => {
      ...
      await client.query(
        `UPDATE rawatan_risiko SET is_deleted = true, deleted_at = NOW() WHERE rawatan_id = $1 AND is_deleted = false`,
        [rawatan_id]
      );
      await client.query(
        `UPDATE pelan_tindakan_rawatan SET is_deleted = true, deleted_at = NOW() WHERE rawatan_id = $1 AND is_deleted = false`,
    ...
    try {
      ...
      await catatAktiviti(user.pengguna_id, "Padam Rawatan", logRingkasan, logPerincian);
    } catch (logErr) {
...
export const rawatanIkutRisiko = async (req, res) => {
  try {
    const { risiko_id } = req.params;
