...
import { adaPenilaian, payloadKemaskiniRisiko } from "./data";
 ...
 * - Pinda penilaian sedia ada: PUT /risiko/:id (tanpa mengundurkan status
 *   pemantauan).
 */
export default function BorangPenilaian({ risiko, onSelesai, onBatal }) {
  ...
  const simpan = async (e) => {
    ...
    try {
      if (pinda) {
        await api.put(`/risiko/${risiko.id}`, payloadKemaskiniRisiko(risiko, skor));
      } else {
        await api.put(`/rawatan/penilaian/${risiko.id}`, { ...skor, tahapRisiko: hasil.tahapRisiko });
      }
      onSelesai(pinda ? "Penilaian risiko dipinda." : "Penilaian risiko disimpan.");
    } catch (err) {
        ...
        <div className="grid content-start gap-4">
          ...
          {pinda && (
            <p className="text-xs text-muted-foreground">
              Meminda penilaian tidak mengubah status log pemantauan sedia ada.
            </p>
          )}
        </div>
