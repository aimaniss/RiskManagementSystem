...
import "./ViewRisikoModal.css"; // Import CSS baharu
import api from "../../api/api";
import PanduanModal from "../Panduan/Panduan";
import TambahLogModal from "./TambahLogModal"; // Dikekalkan untuk 'papar' log
...
export default function ViewRisikoModal({ isOpen, risk, onClose }) {
  ...
  // ⭐️ DIUBAH: State untuk modal log (kini hanya untuk 'papar')
  ...
  // ⭐️ DIBUANG: Logik 'userRole' tidak diperlukan untuk 'view'
  // ⭐️ DIBUANG: Logik 'canViewTindakanColumn'

  const fetchLog = async (risikoId) => {
    if (!risikoId) return;
    setIsLoadingLog(true);
    try {
      // Menggunakan 'sejarah' kerana ia kekal 'read-only'
  ...
  // ⭐️ DIBUANG: handleDeleteLog
  // ⭐️ DIBUANG: handleEditLog
  // ⭐️ DIBUANG: handleLogSaved

  // ⭐️ DIKEKALKAN: Fungsi untuk 'papar' log
  const handleViewLog = (logItem) => {
    setLogToView(logItem);
    setIsLogModalOpen(true);
  };

  // ⭐️ DIKEKALKAN: Fungsi untuk tutup modal log
      ...
      >
                  ...
                  <table className="viewrisiko-log-table">
                    ...
                    <tbody>
                      {logData.length > 0 ? (
                        ...
                        <tr>
                          <td
                            colSpan={13} // ⭐️ DIUBAH: colSpan 13
        ...
        {isLogModalOpen && (
          <TambahLogModal
            ...
            onSaveSuccess={null} // ⭐️ DIBUANG: Tiada 'save'
            logDataToEdit={logToView}
            mode="papar" // ⭐️ DIKEKALKAN: Hardcoded 'papar'
            // ⭐️ DIBUANG: userRole tidak diperlukan
          />
        )}
      </div>
    </div>
