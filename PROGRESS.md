...
|--------|------|--------------------|
| 2026-09-26 | Laporan: Analisis | Tab "Analitik" dinamakan semula **Analisis** (kata nama BM; endpoint `/laporan/analitik` kekal), fail `AnalisisRisiko.jsx`/`analisis.js`. Komponen carta gaya shadcn/ui `components/ui/chart.jsx` (atas Recharts). Carta baharu: donut taburan tahap, kawasan trend risiko aktif, radial kadar rawatan berkesan, radar profil kategori; carta sedia ada digaya semula (petunjuk ikut turutan tahap, tooltip seragam). Spec 16 dikemas kini. E2E **100/100** |
| 2026-09-26 | Log masuk | Halaman log masuk dua panel: kiri biru padu (logo besar, nama sistem & unit sahaja), kanan borang terus di atas latar putih (tajuk 28px, medan & butang 46px, teks bantuan dua baris di bawah garis pemisah), butang biru tua & kaki halaman versi; pada telefon hanya borang berlogo; panel gradien & teks promosi dibuang; "Ingat saya" kini berfungsi (simpan ID Staf sahaja dalam `localStorage` `ingat_staff_id`, bukan kata laluan). Gaya medan/butang dikongsi dengan Tukar Kata Laluan. E2E **99/99** |
| 2026-09-26 | Senarai Risiko | Kad ringkasan seragam (Jumlah + 4 tahap berlencana R/S/T/ST; klik kad tahap untuk tapis, kiraan tidak jadi sifar), bar tapisan satu baris sentiasa kelihatan (ganti butang Tapisan; grid 2 lajur pada telefon), lajur **Bil**, halaman 10/25/50 rekod dengan butang sebelum/seterusnya (kembali ke halaman 1 bila tapisan/susunan berubah). E2E **99/99** |
| 2026-09-26 | Skala & latar | Skrin komputer riba (1024–1680px): saiz asas `html` 14px (~88%) supaya tidak perlu zum keluar; skrin besar kekal 16px. Latar halaman kelabu sangat cerah (`#f6f8fb`), sidebar putih tanpa garis pemisah di bawah logo. E2E **99/99** |
| 2026-09-26 | Log Aktiviti & sidebar | **Log Aktiviti** direka semula: jadual dikelompok ikut hari, lencana aktiviti berikon & berwarna (`constants/jenisAktiviti.js`, dikongsi tab Sejarah), bar tapisan padat (julat pantas Hari ini/7/30 hari/tersuai), "Sembunyikan log masuk/keluar" lalai (API `sorokSesi`), kad pada telefon, Sheet butiran. **Sidebar** putih di atas latar halaman kelabu sangat cerah (`#f6f8fb`, AppLayout) dengan menu berkumpulan Utama/Risiko/Pentadbiran, item aktif biru padu, lencana bilangan Senarai Tugasan, Log Keluar merah berbingkai. **Mod gelap Tailwind** kini ikut kelas `.dark` (`@custom-variant dark`) — sebelum ini `dark:` ikut tetapan OS. Tajuk halaman 22px. E2E **99/99** |
| 2026-09-26 | Tipografi | Tajuk halaman (`PageHeader`) guna Poppins (utiliti `font-heading`, sepadan jenama log masuk); deskripsi kelabu lembut. Inter & Poppins dimuat sekali di `index.html` (import fon berselerak dalam CSS halaman & Roboto yang tidak digunakan dibuang) |
